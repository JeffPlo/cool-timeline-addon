...
function ctl_metabox_add() {
    ...
    if ( function_exists( 'add_meta_box' ) ) {
        if ( function_exists( 'get_post_types' ) ) {
            ...
            add_meta_box(
                'ctl-select-persons',
                __( 'Select persons', 'cool-timeline' ),
                'ctl_select_persons',
                'cool_timeline',
                'side',
                'low'
...
function ctl_select_persons() {
    global $post;

    $output = array();
    $blogusers = get_users( 'orderby=nicename&role__not_in=Administrator' );
    // Array of WP_User objects.
    if ( is_array( $blogusers ) && !empty( $blogusers ) ) {
        $output[] = '<div class="select-persons widefat">';

        $postMeta = get_post_meta($post->ID, 'select-persons', true);
        foreach ( $blogusers as $user ) {
            $id = 'id="person-' . $user->ID . '" ';
            $name = 'name="select-persons[' . $user->ID . ']" ';
            $value = 'value="' . $user->display_name . '" ';
            $checked = is_array( $postMeta ) && in_array( $user->display_name, $postMeta) ? 'checked="checked" ' : '';
            $output[] = '<label for="person-' . $user->ID . '">';
            $output[] = '<input type="checkbox"' . $id . $name . $value . $checked . '>' . $user->display_name;
            $output[] = '</label>';
        }
        $output[] = '</div>';
    }
        ...
        if ( $ct_metabox['type'] == 'text' ) {
            ...
            if ( $array ) {
                ...
                foreach ( $array as $id => $option ) {
                    ...
                    if ( isset( $ct_metabox['default'] ) ) {
                        if ( $ct_metabox['default'] == $option && empty( $ct_metaboxvalue ) ) {
            ...
            if ( $array ) {
                ...
                foreach ( $array as $id => $option ) {
                    ...
                    if ( isset( $ct_metabox['default'] ) ) {
                        if ( $ct_metabox['default'] == $id && empty( $ct_metaboxvalue ) ) {
                            $selected = 'selected="selected"';
                        } else {
                            $selected = '';
                        }
            ...
            foreach ( $ct_metabox['options'] as $key => $option ) {
                ...
                $selected = '';
                if ( $ct_metaboxvalue != '' ) {
                    if ( $ct_metaboxvalue == $key ) {
                        $checked = ' checked';
                        $selected = 'ctl-meta-radio-img-selected';
                    }
                } else {
...
function ctl_metabox_handle( $post_id ) {
    ...
    if ( ( get_post_type() != '' ) && ( get_post_type() != 'nav_menu_item' ) && ( get_post_type() == 'cool_timeline' ) && wp_verify_nonce( $_POST['weo-custom-fields-nonce'], 'weo-custom-fields' ) ) {
        $persons = isset( $_POST['select-persons'] ) ? $_POST['select-persons'] : '';
        if ( get_option( 'select-persons' ) != $persons ) update_post_meta( $pID, 'select-persons', $persons );
