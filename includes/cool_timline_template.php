...
if ( !class_exists( 'CoolTimeline_Template' ) ) {
    ...
    class CoolTimeline_Template {
        ...
        function cooltimeline_register_shortcode() {
            ...
            add_shortcode( 'cool-timeline-legend', array( &$this, 'cooltimeline_legend' ) );
        }
            ...
            */
            ...
            if ( $ctl_loop->have_posts() ) {
                ...
                while ( $ctl_loop->have_posts() ) : $ctl_loop->the_post();
                    ...
                    $type = get_post_meta( $post->ID, 'fa_type', true );
                    ...
                    $ctl_html .= '<div id="story-' . $p_id . '" class="timeline-post ' . $type . ' '. $even_odd . ' ' . $post_skin_cls . ' ' . $clt_icons . ' post-' . $p_id . ' ' . $compt_cls . '">';
                    if ( $layout != "compact" ) {
                    ...
                    $persons = get_post_meta( $post->ID, 'select-persons', true );
                    if( !empty( $persons ) ) {
                        $ctl_html .= '<div class="content-persons">' . join(', ', $persons ) . '</div>';
                    }
 ...
 ================================================== -->';
        ...
        function cooltimeline_legend( $atts, $content = null ) {
            $output = array();

            $output[] = '<ul class="cool-timeline-legend">';
            $output[] = '<li class="first-color-background">' . __( 'Senioren', 'cool-timeline' ) . '</li>';
            $output[] = '<li class="second-color-background">' . __( 'Jugend', 'cool-timeline' ) . '</li>';
            $output[] = '<li class="third-color-background">' . __( 'Schulschach', 'cool-timeline' ) . '</li>';
            $output[] = '</ul>';
